...
async function generateRagResponse(userMessage, sessionId) {
  ...
  const finalPrompt = `
    ...
    1.  まず、【ユーザーからの最後の質問】がどの言語（Japanese/English）で書かれているか判断してください。
    2.  次に、【お店の知識ベース】の中から、【ユーザーからの最後の質問】に意味的に最も関連性の高い項目を1つだけ見つけてください。
    3.  もし関連する項目が見つかった場合は、その項目を参考に、質問された言語で簡潔な回答を生成してください。URLや手順などの重要な情報は省略しないでください。
    4.  もし関連する項目が全く見つからない場合は、質問された言語で「申し訳ありません、その質問にはお答えできません。」とだけ回答してください。
    5.  回答には余計な言葉や会話的な表現（「〜ですね」など）は含めないでください。
